...
class Graph (f: String) {
	...
	def computeCentrality() {
		...
		// create a list with strings to return in the API
		centralityJSON = List[JsValue]()
		for(c <- centrality){
		  centralityJSON = (centralityJSON :+ c.toJson)
		}
	...
	var centralityJSON: List[JsValue] = List[JsValue]()
